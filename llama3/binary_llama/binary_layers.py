...
from .binary_functions import BinaryLinearFunction
from .state_storage import Config
...
# %% Linear Functions
class BinaryColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        super(BinaryColumnParallelLinear, self).__init__()
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        input_parallel = copy_to_model_parallel_region(input_)
        # Matrix multiply.
        output_parallel = BinaryLinearFunction.apply(input_parallel, self.weight, self.bias, self.kk, self.aa)
...
class BinaryRowParallelLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        super(BinaryRowParallelLinear, self).__init__()
    ...
    def set_kk_stage2(self, ratio):
        with torch.no_grad():
            ...
            # Sample approximately 10% of the indices that meet the condition
            total_elements = weight_indice.sum().item()
            sample_size = int(total_elements * 0.25)
            selected_indices = torch.randint(0, total_elements, (sample_size,))
            # Using sampled indices to calculate quantile
            quantile_value = torch.quantile(weight_flatten[selected_indices], q).to(ratio.device, ratio.dtype)
            # Update kk
            ...
            if self.kk < 1:
                ...
                self.aa = Parameter(torch.tensor([1.]).to(self.kk.device), requires_grad=False)
